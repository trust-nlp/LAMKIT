...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_from_disk("/aul/homes/hzhen011/datasets/lex-glue/train")
        #train_dataset = train_dataset.shuffle(seed=42).select(range(int(len(train_dataset) * 0.05)))
    if training_args.do_eval:
        eval_dataset = load_from_disk("/aul/homes/hzhen011/datasets/lex-glue/validation")
        #eval_dataset = eval_dataset.shuffle(seed=42).select(range(int(len(eval_dataset) * 0.1)))
    if training_args.do_predict:
        predict_dataset = load_from_disk("/aul/homes/hzhen011/datasets/lex-glue/test")
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # Add an additional column for 'none' label probabilities
        y_probs_none = np.zeros((y_probs.shape[0], y_probs.shape[1] + 1))
        y_probs_none[:, :-1] = y_probs
        y_probs_none[:, -1] = 1 - np.max(y_probs, axis=1)  # Probability of 'none' label

        # Calculate AUC
        macro_auc = roc_auc_score(y_true, y_probs_none, average='macro')
        micro_auc = roc_auc_score(y_true, y_probs_none, average='micro')
