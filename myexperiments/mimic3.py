...
from CLtrainer import CurriculumLearningMultilabelTrainer
...
from LAMKIT import HierarchicalConvolutionalBert
...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("json", data_files="/aul/homes/hzhen011/datasets/mimic_train50.json",name=data_args.task)
        train_dataset=train_dataset['train']
        #train_dataset = train_dataset.shuffle(seed=42).select(range(int(len(train_dataset) * 0.01)))      
    if training_args.do_eval:
        eval_dataset = load_dataset("json", data_files="/aul/homes/hzhen011/datasets/mimic_dev50.json",name=data_args.task)
        eval_dataset=eval_dataset['train']
        #eval_dataset = eval_dataset.shuffle(seed=42).select(range(int(len(eval_dataset) * 0.01)))        
    if training_args.do_predict:
        predict_dataset = load_dataset("json", data_files="/aul/homes/hzhen011/datasets/mimic_test50.json",name=data_args.task)
    ...
    # Labels
    with open("/aul/homes/hzhen011/caml-mimic-fixed-/mimicdata/mimic3/TOP_50_CODES.csv", 'r') as f:
        reader = csv.reader(f)
        label_list = []
        for row in reader:
...
# define encoder
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        y_probs = expit(logits)

        # For multi-label, AUC is calculated for each label and then averaged
        # This is the macro AUC calculation for multi-label
        macro_auc = roc_auc_score(y_true, y_probs, average='macro')

        # Micro AUC for multi-label aggregates all the TPs, FPs, FNs, and TNs
    ...
    if model_args.curriculum:
        trainer = CurriculumLearningMultilabelTrainer(
                model=model,
                args=training_args,
