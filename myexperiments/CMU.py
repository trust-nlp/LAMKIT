...
from LAMKIT.myexperiments.LAMKIT import HierarchicalConvolutionalBert
...
def main():
    ...
    if data_args.task=='CMU':

        if training_args.do_train:
            train_dataset = load_from_disk("/hdd18t/han/datasets/CMUBookSummary/train")
            #train_dataset = train_dataset.shuffle(seed=42).select(range(int(len(train_dataset) * 0.05)))
        if training_args.do_eval:
            eval_dataset = load_from_disk("/hdd18t/han/datasets/CMUBookSummary/dev")
            #eval_dataset = eval_dataset.shuffle(seed=42).select(range(int(len(eval_dataset) * 0.1)))
        if training_args.do_predict:
            predict_dataset = load_from_disk("/hdd18t/han/datasets/CMUBookSummary/test")
            #predict_dataset = predict_dataset.shuffle(seed=42).select(range(int(len(predict_dataset) * 0.1)))
    elif data_args.task=='PairdCMU':
        if training_args.do_train:
            train_dataset = load_from_disk("/hdd18t/han/datasets/PairdCMUBookSummary/train")
            #train_dataset = train_dataset.shuffle(seed=42).select(range(int(len(train_dataset) * 0.05)))
        if training_args.do_eval:
            eval_dataset = load_from_disk("/hdd18t/han/datasets/PairdCMUBookSummary/dev")
            #eval_dataset = eval_dataset.shuffle(seed=42).select(range(int(len(eval_dataset) * 0.1)))
        if training_args.do_predict:
            predict_dataset = load_from_disk("/hdd18t/han/datasets/PairdCMUBookSummary/test")
            #predict_dataset = predict_dataset.shuffle(seed=42).select(range(int(len(predict_dataset) * 0.1)))   
    else:
        print('Wrong downstream task') 
    # Labels
    label_list = list(range(227))
