...
def main():
    ...
    dataset = load_dataset('json', data_files='/aul/homes/hzhen011/datasets/diabetes2.json')
    dataset_length = len(dataset['train'])
    indices = list(range(dataset_length))
    train_indices, temp_test_indices = train_test_split(indices, test_size=0.3, random_state=42)
    test_indices, val_indices= train_test_split(temp_test_indices, test_size=0.5, random_state=42)
...
# define encoder
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # Initialize AUC scores list for macro AUC
        macro_auc_scores = []

        # Calculate AUC for each label
        for i in range(y_probs.shape[1]):
            # Check if the label is present in y_true
            if np.any(y_true[:, i]):
                macro_auc_scores.append(roc_auc_score(y_true[:, i], y_probs[:, i]))

        # Compute macro AUC
        macro_auc = np.mean(macro_auc_scores) if macro_auc_scores else 0.5
