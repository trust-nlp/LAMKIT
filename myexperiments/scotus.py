...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_from_disk("/aul/homes/hzhen011/datasets/scotus/train")
        #train_dataset = train_dataset.shuffle(seed=42).select(range(int(len(train_dataset) * 0.05)))
    if training_args.do_eval:
        eval_dataset = load_from_disk("/aul/homes/hzhen011/datasets/scotus/validation")
        #eval_dataset = eval_dataset.shuffle(seed=42).select(range(int(len(eval_dataset) * 0.1)))
    if training_args.do_predict:
        predict_dataset = load_from_disk("/aul/homes/hzhen011/datasets/scotus/test")
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        y_true = np.array(p.label_ids, dtype=np.int32)
        ...
        micro_auc = roc_auc_score(y_true=y_true_binarized, y_score=probabilities, average='micro',multi_class='ovo')
        all_possible_labels = np.arange(14)  # Labels from 0 to 13
        unique_classes = np.unique(y_true)

        # Identify labels that are not present in y_true
        missing_labels = np.setdiff1d(all_possible_labels, unique_classes)

        # Binarize y_true
        y_true_binarized = label_binarize(y_true, classes=all_possible_labels)

        # Drop columns corresponding to missing labels
        for label in missing_labels:
            idx = np.where(all_possible_labels == label)[0][0]
            y_true_binarized = np.delete(y_true_binarized, idx, axis=1)
            probabilities = np.delete(probabilities, idx, axis=1)

        # Calculate AUC for each remaining class
        auc_scores = []
        for i in range(y_true_binarized.shape[1]):
            class_auc = roc_auc_score(y_true_binarized[:, i], probabilities[:, i])
